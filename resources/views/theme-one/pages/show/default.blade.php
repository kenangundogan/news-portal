    ...
    <div>
        ...
        <div class="relative overflow-hidden h-[calc(100vh-214px)] md:h-[calc(100vh-80px)] mb-4 md:mb-10 before:content-[''] before:absolute before:z-10 before:w-full before:h-full before:bg-gradient-to-t before:from-black/60">
            <div class="absolute z-20 left-0 right-0 m-auto bottom-4 md:bottom-10 max-w-3xl text-center p-4 overflow-hidden break-words">
                ...
                <div class="text-3xl md:text-6xl font-bold mb-2 text-white font-sans line-clamp-3">{{ $news->title }}</div>
                <div class="text-md md:text-xl text-white mb-2 line-clamp-2">{{ $news->description }}</div>
        ...
        <div class="mb-10 px-4 pb-4">
            @foreach ($news->contents as $content)
                    ...
                    @elseif ($content->contentType->name == 'Parallax Image')
                        <div class="relative overflow-hidden mb-10">
                            <div class="h-[500px] bg-fixed bg-center bg-cover before:content-[''] before:absolute before:w-full before:h-full before:bg-black/30" style="background-image: url('{{ asset($content->content) }}')">
                    ...
                    @elseif ($content->contentType->name == 'Link')
                        <div class="mb-4 max-w-3xl m-auto">
                            <a target="_blank" href="{{ $content->content }}" class="hover:underline block mb-4">{{ $content->content }}</a>
                        </div>
                    @endif
            @endforeach
        </div>
